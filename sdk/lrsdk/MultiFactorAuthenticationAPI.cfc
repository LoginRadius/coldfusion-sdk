<cfcomponent displayname="multifactorauthenticationapi" 
             hint="This is the CFC for Multi Factor Authentication APIs">
	...
	<cffunction name="init" hint="sets up an instance of the component" output="false">
		...
		<cfargument name="lr_api_secret" required="true" hint="API secret provided by LoginRadius" 
		            type="string"/>
		<cfargument name="LR_MULTI_FACTOR_AUTH_API_ENDPOINT" required="false" 
		            default="https://api.loginradius.com/identity/v2/auth" 
		            hint="LoginRadius multi factor endpoint" type="string"/>
	...
	<cffunction name="mfaEmailLogin" 
	            hint="This API can be used to login by emailid on a Multi-Factor Authentication enabled LoginRadius site.">
		<!--- Define arguments. --->
	...
	<cffunction name="mfaUserNameLogin" 
	            hint="This API can be used to login by username on a Multi-Factor Authentication enabled LoginRadius site.">
		<!--- Define arguments. --->
		<cfargument name="payload" type="any" required="true" hint="data in json format"/>
		<cfargument name="loginurl" type="string" required="false" default=""
		            hint="login url"/>
	...
	<cffunction name="mfaPhoneLogin" 
	            hint="This API is used to log in by phone on Multi-Factor Authentication enabled LoginRadius site.">
		<!--- Define arguments. --->
		<cfargument name="payload" type="any" required="true" hint="data in json format"/>
		<cfargument name="loginurl" type="string" required="false" default=""
		            hint="login url"/>
	...
	<cffunction name="mfaValidateToken" 
	            hint="This API is used to configure the Multi-factor authentication after login by using the access_token when MFA is set as optional on the LoginRadius site.">
		<!--- Define arguments. --->
	...
	<cffunction name="mfaBackupCodeByToken" 
	            hint="This API is used to get a set of backup codes via access token.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true"/>
	...
	<cffunction name="mfaResetBackupCodeByToken" 
	            hint="This API is used to reset the backup codes on a given account via the access token.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true"/>
	...
	<cffunction name="mfaValidateBackupCode" hint="This API is used to validate the backup code.">
		...
		<cfhttp method="PUT" 
		        url="#variables.LR_MULTI_FACTOR_AUTH_API_ENDPOINT#/login/2fa/verification/backupcode">
			<cfhttpparam type="header" name="Accept-Encoding" value="*"/>
			<cfhttpparam type="Header" name="TE" value="deflate;q=0">
			<cfhttpparam type="header" name="Content-Type" value="application/json"/>
			<cfhttpparam name="apikey" value="#variables.lr_api_key#" type="URL">
			<cfhttpparam value="#payload#" type="body">
			<cfhttpparam name="secondfactorauthenticationtoken" 
			             value="#arguments.secondfactorauthenticationtoken#" type="URL">
		</cfhttp>
	
		<cfreturn loginradiusGetResponse(cfhttp)>
	</cffunction>
	
	<cffunction name="mfaValidateOTP" 
	            hint="This API is used to login via Multi-factor authentication by passing the One Time Password received via SMS.">
		<!--- Define arguments. --->
		<cfargument name="secondfactorauthenticationtoken" type="string" required="true"/>
		<cfargument name="payload" type="any" required="true" hint="data in json format"/>
		<cfargument name="smstemplate2fa" type="string" required="false" default=""
		            hint="sms template two factor"/>
	
		<cfhttp method="PUT" 
		        url="#variables.LR_MULTI_FACTOR_AUTH_API_ENDPOINT#/login/2fa/verification/otp">
			<cfhttpparam type="header" name="Accept-Encoding" value="*"/>
			<cfhttpparam type="Header" name="TE" value="deflate;q=0">
			<cfhttpparam type="header" name="Content-Type" value="application/json"/>
			<cfhttpparam name="apikey" value="#variables.lr_api_key#" type="URL">
	...
	<cffunction name="mfaValidateGoogleAuthCode" 
	            hint="This API is used to login via Multi-factor authentication by passing the google authenticator code.">
		<!--- Define arguments. --->
		<cfargument name="secondfactorauthenticationtoken" type="string" required="true"/>
		...
		<cfhttp method="PUT" 
		        url="#variables.LR_MULTI_FACTOR_AUTH_API_ENDPOINT#/login/2fa/verification/googleauthenticatorcode">
			<cfhttpparam type="header" name="Accept-Encoding" value="*"/>
			<cfhttpparam type="Header" name="TE" value="deflate;q=0">
			<cfhttpparam type="header" name="Content-Type" value="application/json"/>
			<cfhttpparam name="apikey" value="#variables.lr_api_key#" type="URL">
	...
	<cffunction name="mfaUpdatePhone" 
	            hint="This API is used to update the Multi-factor authentication phone number.">
		<!--- Define arguments. --->
		<cfargument name="secondfactorauthenticationtoken" type="string" required="true" 
		            hint="second factor authentication token"/>
		<cfargument name="phoneno2fa" type="string" required="true" hint="phoneno2fa"/>
		<cfargument name="smstemplate2fa" type="string" required="false" default=""
		            hint="sms template multi factor"/>
	...
	<cffunction name="mfaUpdatePhoneByToken" 
	            hint="This API is used to update the Multi-factor authentication phone number.">
		<!--- Define arguments. --->
		<cfargument name="phoneno2fa" type="string" required="true" hint="phoneno2fa"/>
	...
	<cffunction name="updateMfaByGoogleAuthCode" 
	            hint="This API is used to Enable Multi-factor authentication by access token on user login.">
		<!--- Define arguments. --->
		<cfargument name="googleauthenticatorcode" type="string" required="true" 
		            hint="googleauthenticatorcode"/>
		<cfargument name="accesstoken" type="string" required="true" hint="access token"/>
		<cfargument name="smstemplate" type="string" required="false" default=""
		            hint="sms template"/>
		...
		<cfhttp method="PUT" 
		        url="#variables.LR_MULTI_FACTOR_AUTH_API_ENDPOINT#/account/2fa/verification/googleauthenticatorcode">
			<cfhttpparam type="header" name="Accept-Encoding" value="*"/>
			<cfhttpparam type="Header" name="TE" value="deflate;q=0">
			<cfhttpparam type="header" name="Content-Type" value="application/json"/>
			<cfhttpparam type="header" name="Authorization" value="Bearer #arguments.accesstoken#">
	...
	<cffunction name="updateMfaByOtp" 
	            hint="This API is used to trigger the Multi-factor authentication settings after login for secure actions.">
		<!--- Define arguments. --->
		<cfargument name="payload" type="any" required="true" hint="data in json format"/>
		<cfargument name="accesstoken" type="string" required="true" hint="access token"/>
	
		<cfhttp method="PUT" 
		        url="#variables.LR_MULTI_FACTOR_AUTH_API_ENDPOINT#/account/2fa/verification/otp">
			<cfhttpparam type="header" name="Accept-Encoding" value="*"/>
			<cfhttpparam type="Header" name="TE" value="deflate;q=0">
			<cfhttpparam type="header" name="Content-Type" value="application/json"/>
			<cfhttpparam type="header" name="Authorization" value="Bearer #arguments.accesstoken#">
	...
	<cffunction name="mfaResetGoogleAuthenticatorByToken" 
	            hint="This API Resets the Google Authenticator configurations on a given account via the access token.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true" hint="access token"/>
	...
	<cffunction name="mfaResetSMSAuthenticatorByToken" 
	            hint="This API Resets the SMS Authenticator configurations on a given account via the access token.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true" hint="access token"/>
	...
	<cffunction name="mfaReAuthenticate" 
	            hint="This API is used to trigger the Multi-Factor Autentication workflow for the provided access token.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true" hint="access token"/>
		<cfargument name="smstemplate2fa" type="string" required="false" default=""
		            hint="sms template 2fa"/>
	...
	<cffunction name="validateMfaByGoogleAuthCode" 
	            hint="This API is used to re-authenticate via Multi-factor-authentication by passing the google authenticator code.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true" hint="access token"/>
		...
		<cfhttp method="PUT" 
		        url="#variables.LR_MULTI_FACTOR_AUTH_API_ENDPOINT#/account/reauth/2fa/googleauthenticatorcode">
			<cfhttpparam type="header" name="Accept-Encoding" value="*"/>
			<cfhttpparam type="Header" name="TE" value="deflate;q=0">
			<cfhttpparam type="header" name="Content-Type" value="application/json"/>
			<cfhttpparam type="header" name="Authorization" value="Bearer #arguments.accesstoken#">
	...
	<cffunction name="validateMfaByBackupCode" 
	            hint="This API is used to re-authenticate by set of backup codes via access token.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true" hint="access token"/>
		...
		<cfhttp method="PUT" 
		        url="#variables.LR_MULTI_FACTOR_AUTH_API_ENDPOINT#/account/reauth/2fa/backupcode">
			<cfhttpparam type="header" name="Accept-Encoding" value="*"/>
			<cfhttpparam type="Header" name="TE" value="deflate;q=0">
			<cfhttpparam type="header" name="Content-Type" value="application/json"/>
			<cfhttpparam type="header" name="Authorization" value="Bearer #arguments.accesstoken#">
	...
	<cffunction name="validateMfaByOtp" 
	            hint="This API is used to re-authenticate via Multi-factor authentication by passing the One Time Password received via SMS.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true" hint="access token"/>
		<cfargument name="payload" type="any" required="true" hint="data in json format"/>
		<cfargument name="smstemplate2fa" type="string" required="false" default=""
	...
	<cffunction name="validateMfaByPassword" 
	            hint="This API is used to re-authenticate via Multi-factor-authentication by passing the password.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true" hint="access token"/>
		<cfargument name="payload" type="any" required="true" hint="data in json format"/>
		<cfargument name="smstemplate2fa" type="string" required="false" default=""
