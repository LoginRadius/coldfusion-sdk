<cfcomponent displayname="authenticationapi" 
             hint="This is the CFC for Customer Identity Management APIs">
	...
	<cffunction name="init" hint="sets up an instance of the component" output="false">
		...
		<cfargument name="LR_USER_AUTH_API_ENDPOINT" required="false" 
		            default="https://api.loginradius.com/identity/v2/auth" 
		            hint="LoginRadius user registration endpoint" type="string"/>
	...
	<cffunction name="getSocialIdentity" 
	            hint="This API is called just after account linking API and it prevents the raas profile of the second account from getting created">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true"/>
	...
	<cffunction name="validateAccessToken" 
	            hint="This api validates access token, if valid then returns a response with its expiry otherwise error.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true"/>
	...
	<cffunction name="deleteAccount" 
	            hint="This API is used to delete an account by passing it a delete token.">
		<!--- Define arguments. --->
	...
	<cffunction name="invalidateAccessToken" 
	            hint="This api call invalidates the active access token or expires an access token's validity.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true"/>
	...
	<cffunction name="securityQuestionByToken" 
	            hint="This API is used to retrieve the list of questions that are configured on the respective LoginRadius site.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true"/>
	...
	<cffunction name="securityQuestionByEmail" 
	            hint="This API is used to retrieve the list of questions that are configured on the respective LoginRadius site.">
		<!--- Define arguments. --->
		<cfargument name="email" type="string" required="true"/>
	...
	<cffunction name="securityQuestionByUsername" 
	            hint="This API is used to retrieve the list of questions that are configured on the respective LoginRadius site.">
		<!--- Define arguments. --->
		<cfargument name="username" type="string" required="true"/>
	...
	<cffunction name="securityQuestionByPhone" 
	            hint="This API is used to retrieve the list of questions that are configured on the respective LoginRadius site.">
		<!--- Define arguments. --->
	...
	<cffunction name="changePassword" 
	            hint="This API is used to change the accounts password based on the previous password.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true" hint="access token"/>
	...
	<cffunction name="accountLink" 
	            hint="This API is used to link up a social provider account with the specified account based on the access token and the social providers user access token.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true" hint="access token"/>
	...
	<cffunction name="resendEmailVerification" 
	            hint="This API resends the verification email to the user.">
		<!--- Define arguments. --->
		<cfargument name="email" type="string" required="true" hint="email"/>
		<cfargument name="verificationurl" type="string" required="false" default=""
		            hint="verification url"/>
	...
	<cffunction name="resetPasswordByResetToken" 
	            hint="This API is used to set a new password for the specified account.">
		<!--- Define arguments. --->
	...
	<cffunction name="resetPasswordByOTP" 
	            hint="This API is used to set a new password for the specified account.">
		<!--- Define arguments. --->
		<cfargument name="email" type="string" required="true" hint="email"/>
		<cfargument name="password" type="string" required="true" hint="password"/>
	...
	<cffunction name="resetPasswordByEmail" 
	            hint="This API is used to reset password for the specified account by security question.">
		<!--- Define arguments. --->
		<cfargument name="payload" type="any" required="true" hint="data in json format"/>
	...
	<cffunction name="resetPasswordByPhone" 
	            hint="This API is used to reset password for the specified account by security question.">
		<!--- Define arguments. --->
		<cfargument name="payload" type="any" required="true" hint="data in json format"/>
	...
	<cffunction name="resetPasswordByUsername" 
	            hint="This API is used to reset password for the specified account by security question.">
		<!--- Define arguments. --->
		<cfargument name="payload" type="any" required="true" hint="data in json format"/>
	...
	<cffunction name="updateSecurityQuestionByToken" 
	            hint="This API is used to update security questions by the access token.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true" hint="access token"/>
		<cfargument name="payload" type="any" required="true" hint="data in json format"/>
	...
	<cffunction name="deleteAccountByEmailConfirmation" 
	            hint="This API deletes a user account by passing the user's access token.">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true"/>
		<cfargument name="deleteurl" type="string" required="false" default=""
		            hint="delete url"/>
		<cfargument name="emailtemplate" type="string" required="false" default=""
		            hint="email template"/>
	...
	<cffunction name="removeEmail" 
		...
		<cfset result = myObj.delete(requestURL, "#arguments.accesstoken#", payload, "", 
	...
	<cffunction name="accountUnlink" 
	            hint="This API is used to unlink up a social provider account with the specified account based on the access token and the social providers user access token..">
		<!--- Define arguments. --->
		<cfargument name="accesstoken" type="string" required="true" hint="access token"/>
	...
	<cffunction name="passwordLessLoginByEmail" 
	            hint="This API is used to send Passwordless Login verification link by Email ID.">
		<!--- Define arguments. --->
		<cfargument name="email" type="string" required="true"/>
		<cfargument name="passwordlesslogintemplate" type="string" required="false" default=""
		            hint="passwordless login template"/>
		<cfargument name="verificationurl" type="string" required="false" default=""
		            hint="verification url"/>
		...
		<cfhttp url="#variables.LR_USER_AUTH_API_ENDPOINT#/login/passwordlesslogin/email">
			...
			<cfhttpparam name="apikey" value="#variables.lr_api_key#" type="URL">
			<cfhttpparam name="email" value="#arguments.email#" type="URL">
			<cfhttpparam name="passwordlesslogintemplate" value="#arguments.passwordlesslogintemplate#" 
			             type="URL">
			<cfhttpparam name="verificationurl" value="#arguments.verificationurl#" type="URL">
		</cfhttp>
		<!--- Get Response --->
	...
	<cffunction name="passwordLessLoginByUsername" 
	            hint="This API is used to send Passwordless Login verification link by UserName.">
		<!--- Define arguments. --->
		<cfargument name="username" type="string" required="true"/>
		<cfargument name="passwordlesslogintemplate" type="string" required="false" default=""
		            hint="passwordless login template"/>
	...
	<cffunction name="passwordLessLoginVerification" 
	            hint="This API is used to verify Passwordless Login verification link.">
		<!--- Define arguments. --->
		<cfargument name="verificationtoken" type="string" required="true"/>
		<cfargument name="welcomeemailtemplate" type="string" required="false" default=""
		            hint="welcome email template"/>
	...
	<cffunction name="phoneLoginUsingOtp" 
	            hint="This API verifies an account by OTP and allows the user to login.">
		<!--- Define arguments. --->
		<cfargument name="payload" type="any" required="true" hint="data in json format"/>
		<cfargument name="smstemplate" type="string" required="false" default=""
		            hint="sms template"/>
		...
		<cfhttp method="PUT" 
		        url="#variables.LR_USER_AUTH_API_ENDPOINT#/login/passwordlesslogin/otp/verify">
			<cfhttpparam type="header" name="Accept-Encoding" value="*"/>
			<cfhttpparam type="Header" name="TE" value="deflate;q=0">
			<cfhttpparam type="header" name="Content-Type" value="application/json"/>
			<cfhttpparam name="apikey" value="#variables.lr_api_key#" type="URL">
	...
	<cffunction name="smartLoginByEmail" hint="This API sends smart login link to the user's Email Id.">
		...
		<cfargument name="smartloginemailtemplate" type="string" required="false" default=""
		            hint="smart login email template"/>
		<cfargument name="welcomeemailtemplate" type="string" required="false" default=""
		            hint="welcome email template"/>
		...
		<cfhttp url="#variables.LR_USER_AUTH_API_ENDPOINT#/login/smartlogin">
			...
			<cfhttpparam name="smartloginemailtemplate" value="#arguments.smartloginemailtemplate#" 
			             type="URL">
			<cfhttpparam name="welcomeemailtemplate" value="#arguments.welcomeemailtemplate#" type="URL">
	...
	<cffunction name="smartLoginByUsername" 
	            hint="This API sends smart login link to the user's Email Id.">
		<!--- Define arguments. --->
		<cfargument name="username" type="string" required="true"/>
		<cfargument name="clientguid" type="string" required="true"/>
		<cfargument name="smartloginemailtemplate" type="string" required="false" default=""
	...
	<cffunction name="smartLoginPing" 
	            hint="This API is used to check that smart login link has been clicked or not on server.">
		<!--- Define arguments. --->
		<cfargument name="clientguid" type="string" required="true"/>
	...
	<cffunction name="oneTouchLoginByEmail" 
	            hint="This API is used to send a link to a specified email for a frictionless login/registration.">
		<!--- Define arguments. --->
		<cfargument name="payload" type="any" required="true" hint="data in json format"/>
		<cfargument name="redirecturl" type="string" required="false" default=""
		            hint="redirecturl"/>
		<cfargument name="onetouchloginemailtemplate" type="string" required="false" default=""
		            hint="one touch login email template"/>
		<cfargument name="welcomeemailtemplate" type="string" required="false" default=""
		            hint="welcome email template"/>
		...
		<cfhttp method="POST" url="#variables.LR_USER_AUTH_API_ENDPOINT#/onetouchlogin/email">
			...
			<cfhttpparam name="apikey" value="#variables.lr_api_key#" type="URL">
			<cfhttpparam name="redirecturl" value="#arguments.redirecturl#" type="URL">
			<cfhttpparam name="onetouchloginemailtemplate" value="#arguments.onetouchloginemailtemplate#" 
			             type="URL">
			<cfhttpparam name="welcomeemailtemplate" value="#arguments.welcomeemailtemplate#" type="URL">
			<cfhttpparam value="#arguments.payload#" type="body">
		</cfhttp>
	...
	<cffunction name="oneTouchLoginByPhone" 
	            hint="This API is used to send a link to a specified phone number for a frictionless login/registration.">
		<!--- Define arguments. --->
		<cfargument name="payload" type="any" required="true" hint="data in json format"/>
		<cfargument name="smstemplate" type="string" required="false" default=""
		            hint="sms template"/>
	...
	<cffunction name="oneTouchOtpVerification" 
	            hint="This API is used to verify the otp for One Touch Login.">
		<!--- Define arguments. --->
		<cfargument name="otp" type="string" required="true" hint="otp"/>
	...
	<cffunction name="validateCode" 
	            hint="This API allows you to validate code for a perticular dropdown member.">
		<!--- Define arguments. --->
		<cfargument name="payload" type="any" required="true" hint="data in json format"/>
